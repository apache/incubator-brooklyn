...
public class TimeWeightedDeltaEnricher<T extends Number> extends AbstractTransformingEnricher<T> {
    ...
    public void onEvent(SensorEvent<T> event, long eventTime) {
        ...
        if (current == null) {
            ...
            if (LOG.isTraceEnabled()) LOG.trace("set {} to {}, {} -> {} at {}", this, deltaPostProcessed, lastValue, current, eventTime);
            return;
        }
        ...
        if (eventTime > lastTime) {
            if (lastValue == null) {
                ...
                if (LOG.isTraceEnabled()) LOG.trace("{} received event but no last value so will not emit, null -> {} at {}", this, current, eventTime); 
            } else {
