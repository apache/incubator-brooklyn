...
public class MediatorGroup extends MontereyTypedGroup implements Balanceable {
    ...
    static MediatorGroup newAllLocationsInstance(MontereyNetworkConnectionDetails connectionDetails, MontereyProvisioner montereyProvisioner, Collection<Location> locs) {
        return new MediatorGroup(connectionDetails, montereyProvisioner, locs, { true } );
    }
    
    MediatorGroup(MontereyNetworkConnectionDetails connectionDetails, MontereyProvisioner montereyProvisioner, Collection<Location> locs, Closure locFilter) {
        super(connectionDetails, montereyProvisioner, Dmn1NodeType.M, locs, locFilter)
    }
