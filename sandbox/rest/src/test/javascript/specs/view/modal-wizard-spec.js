 ...
 */
define([
    "underscore", "jquery", "backbone", "view/modal-wizard", "model/application", "text!tpl/home/step1.html",
    "text!tpl/home/step2.html", "text!tpl/home/step3.html", "text!tpl/home/entity-form.html"
], function (_, $, Backbone, ModalWizard, Application, Step1Html, Step2Html, Step3Html, EntityFormHtml) {
    ...
    var fakeRouter = new Backbone.Router(),
        modal = new ModalWizard({appRouter:fakeRouter});
    ...
    describe('tpl/home/entity-form.html', function () {
        var $form = $(EntityFormHtml);
        it('has input#entity-name', function () {
            expect($form.find('input#entity-name').length).toEqual(1);
        });

        it('has select#entiy-type', function () {
            expect($form.find('select#entity-type').length).toEqual(1);
        });

        it('has div#entity-config with ul and h5', function () {
            var $div = $form.filter('div#entity-config');
            expect($div.length).toEqual(1);
            expect($div.find('h5').length).toEqual(1);
            expect($div.find('ul').length).toEqual(1);
        });

        it('has div#entity-group with two inputs and a button', function () {
            var $div = $form.filter('div#entity-group');
            expect($div.find('input#key').length).toEqual(1);
            expect($div.find('input#value').length).toEqual(1);
            expect($div.find('button#add-config').length).toEqual(1);
        });
    });
