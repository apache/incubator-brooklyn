...
import com.cloudsoftcorp.monterey.brooklyn.example.MontereyWebCluster
...
public class MontereySpringTravel extends AbstractApplication {
    ...
    public MontereySpringTravel(Map props=[:]) {
        ...
        webAppName = Preconditions.checkNotNull(props?.webAppName)
        warFilePath = Preconditions.checkNotNull(props?.warFilePath)
        appBundlePath = Preconditions.checkNotNull(props?.appBundlePath)
        appConfigPath = Preconditions.checkNotNull(props?.appConfigPath)

        OsgiClassLoadingContextFromBundle classLoadingContext = new OsgiClassLoadingContextFromBundle(null, MontereySeamBooking.class.getClassLoader());
        ClassLoadingContext.Defaults.setDefaultClassLoadingContext(classLoadingContext);

        URL bundleUrl = MontereySeamBooking.class.getClassLoader().getResource(APP_BUNDLE_RESOURCE_PATH);
        URL appDescriptorUrl = MontereySeamBooking.class.getClassLoader().getResource(APP_CONFIG_RESOURCE_PATH);
        ...
        montereyNetwork.setConfig(MontereyManagementNode.WEB_USERS_CREDENTIAL, [adminCredential])
