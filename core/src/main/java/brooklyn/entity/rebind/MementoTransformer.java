...
public class MementoTransformer {
	...
	public static <T> T transformIdsToLocations(RebindContext rebindContext, Object value, Class<T> requiredType) {
		if (requiredType.isAssignableFrom(Location.class)) {
			return (T) rebindContext.getLocation((String)value);
		} else if (requiredType.isAssignableFrom(Set.class)) {
			Set<Location> result = Sets.newLinkedHashSet();
			for (String id : (List<String>)value) {
				result.add(rebindContext.getLocation(id));
			}
			return (T) result;
		} else if (requiredType.isAssignableFrom(List.class) || requiredType.isAssignableFrom(Collection.class) || requiredType.isAssignableFrom(Iterable.class)) {
