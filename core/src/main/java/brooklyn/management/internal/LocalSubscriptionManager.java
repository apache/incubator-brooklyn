...
public class LocalSubscriptionManager implements SubscriptionManager {
    private static class Subscription<T> {
        ...
        Predicate<SensorEvent<T>> filter;
        EventListener<T> listener;

        public Subscription(Predicate<Entity> entities, Predicate<SensorEvent<T>> filter, EventListener<T> listener) {
    ...
    public void addSubscription(Predicate<Entity> entities, Predicate<SensorEvent<T>> filter, EventListener listener) {
        subscriptions += new Subscription(entities, filter, listener)
    }
