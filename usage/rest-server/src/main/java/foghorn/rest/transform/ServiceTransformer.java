...
public class ServiceTransformer {
    ...
    public static final Function<? super Entity, ServiceSummary> FROM_ENTITY = new Function<Entity, ServiceSummary>() {
        @Override
        public ServiceSummary apply(Entity entity) {
            return ServiceTransformer.serviceSummary(entity);
        }
    };

    //TODO type checking
    public static ServiceSummary serviceSummary(Entity entity) {
        String deploymentUri = "/v1/deployments/" + entity.getApplicationId();
        String applicationUri = "/v1/applications/" + entity.getApplicationId();
        String deviceUri = deploymentUri + "/devices/" + entity.getParent().getId();
        String serviceUri = deviceUri + "/services/" + entity.getId();
        ...
        ImmutableMap.Builder<String, URI> lb = ImmutableMap.<String, URI>builder()
                .put("self", URI.create(serviceUri))
                .put("deployment", URI.create(deploymentUri))
                .put("device", URI.create(deviceUri))
        ...
        return new ServiceSummary(entity.getId(), entity.getDisplayName(), type, entity.getCatalogItemId(), lb.build());
    }
}
