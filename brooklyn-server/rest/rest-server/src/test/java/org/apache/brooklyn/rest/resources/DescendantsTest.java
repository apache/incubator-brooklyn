...
public class DescendantsTest extends BrooklynRestResourceTest {
    ...
    public void testDescendantsInSimpleDeployedApplication() throws InterruptedException, TimeoutException, JsonGenerationException, JsonMappingException, UniformInterfaceException, ClientHandlerException, IOException {
        ...
        descs = client().resource("/applications/"+application.getApplicationId()+"/descendants")
        ...
        descs = client().resource("/applications/"+application.getApplicationId()
        ...
        Map<String,Object> sensors = client().resource("/applications/"+application.getApplicationId()+"/descendants/sensor/foo"
            + "?typeRegex="+StringEscapes.escapeUrlParam(".*\\.RestMockSimpleEntity"))
        ...
        sensors = client().resource("/applications/"+application.getApplicationId()+"/descendants/sensor/foo")
            .get(new GenericType<Map<String,Object>>() {});
        ...
        sensors = client().resource("/applications/"+application.getApplicationId()+"/"
            + "entities/"+entities.get(1).getId()+"/"
